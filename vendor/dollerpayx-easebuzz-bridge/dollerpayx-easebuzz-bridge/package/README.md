...
Private Node.js / Deno / Edge runtime toolkit that exposes the DollerpayX
...
SabPaisa / AirPay / … gateways with DollerpayX **without** changing anything
on the DollerpayX backend or rewriting their PG integrations.

Wire protocol is byte-for-byte identical to the
[DollerpayX WordPress bridge plugin](../../plugins/dollerpayx-bridge/), which
means DollerpayX's [`WordPressBridgeAdapter`](../pg-adapters/src/adapters/wordpress-bridge.ts)
...
1. Accept an externally-provided `txn_id` (DollerpayX's UUID) and use it as the
...
4. Pick PG credentials per `merchant_id` (no hardcoded single-account
   deployments — DollerpayX's multi-tenant routing requires this).
5. Forward the PG's async webhook to DollerpayX via
...
DollerpayX will hand you a tarball out-of-band:

```bash
# The filename encodes the version; check the SHA-256 against what DollerpayX shared.
...
#   "@dpx/bridge-node": "file:./vendor/bosspay-bridge-node-1.0.0.tgz"
```
...
| ------------------------------ | -------- | ----------------------------------------------------------------------- |
| `BOSSPAY_BRIDGE_SECRET`        | Yes      | Shared HMAC secret. Matches `lender_pgs.credentials.bridge_secret` on DollerpayX. |
| `BOSSPAY_API_BASE`             | No       | Defaults to `https://api.dpxreal.com`. Override for staging.          |
...
import type { BridgeHandlers } from '@dpx/bridge-node';
...
```ts
import { createBossPayBridge } from '@dpx/bridge-node';
import { SupabaseTxnStore } from '@dpx/bridge-node/txnStore/supabase';
...
const bridge = createBossPayBridge({
  ...
  bosspayApiBase: 'https://api.dpxreal.com',
  handlers,
...
import { createBossPayBridge, MemoryTxnStore, toExpress } from '@dpx/bridge-node';
...
app.all('/wp-json/bosspay/v1/*', toExpress({
  ctx: { handlers, txnStore: new MemoryTxnStore(), bosspayApiBase: 'https://api.dpxreal.com', version: '1.0.0' },
  bridgeSecret: process.env.BOSSPAY_BRIDGE_SECRET!,
}));
app.listen(3000);
```
...
### 3. Forward the PG's webhook to DollerpayX
...
## Tell DollerpayX admin

Ask DollerpayX to configure your lender PG row like:
...
DollerpayX will hit `{wp_base_url}/wp-json/bosspay/v1/{collect|payout|status|health}`.
...
curl -v \
  -H "X-DollerpayX-Timestamp: $ts" \
  -H "X-DollerpayX-Bridge-Signature: $sig" \
...
| `501 {"error":"PAYOUT_NOT_IMPLEMENTED"}`    | You didn't implement `createPayout`; DollerpayX will mark the payout failed. |
| `CALLBACK_FORWARD_FAILED`                   | DollerpayX backend unreachable or returning 5xx. Check `BOSSPAY_API_BASE`. |
...
1. DollerpayX sends a new tarball (`bosspay-bridge-node-<version>.tgz`).
2. Verify SHA-256 against the hash DollerpayX shared.
...
- No refunds (not in the DollerpayX bridge contract yet).
- No payout defaults — lender must implement `createPayout` if the PG supports it.
- No React checkout widget — hosted by DollerpayX separately on roadmap.
